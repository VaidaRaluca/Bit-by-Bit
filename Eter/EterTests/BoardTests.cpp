...
namespace EterTests
{
	...
	public:
		...
		TEST_METHOD(TestIsSecondaryDiagonalLine)
		{
			Board board;
			std::string lineColor = "red";
			Card card1{ 1, lineColor, true };
			Card card2{ 2, lineColor, true };
			Card card3{ 4, lineColor, true };

			board.SetDimMax(3);

			board.placeCard(0, 2, card1);
			board.placeCard(1, 1, card2);
			board.placeCard(2, 0, card3);

			Assert::IsTrue(board.isSecondaryDiagonalLine(lineColor), L"There should be a secondary diagonal line for 'red'.");
			Card otherColorCard{ 4, "blue", true };
			board.placeCard(1, 1, otherColorCard);
			Assert::IsFalse(board.isSecondaryDiagonalLine(lineColor), L"There should not be a secondary diagonal line for 'red' after disruption.");
			board.clear();
		}
		...
		TEST_METHOD(TestEliminateCardsOnColumn)
		{
			Board board;
			Card testCard1{ 1, "red", true }, testCard2{ 1, "blue", true }, testCard3{ 2, "red", true };
			size_t x = 0, y = 0;
			...
			x = 2;
			board.placeCard(x, y, testCard3);
			Assert::IsFalse(board.isEmptyCell(0, y), L"Cell (0, 0) should not be empty after placing a card.");
			Assert::IsFalse(board.isEmptyCell(1, y), L"Cell (1, 0) should not be empty after placing a card.");
			Assert::IsFalse(board.isEmptyCell(2, y), L"Cell (2, 0) should not be empty after placing a card.");
			board.eliminateCardsOnColumn(y);
			Assert::IsTrue(board.isEmptyCell(0, y), L"Cell (0, 0) should be empty after eliminating cards in the column.");
			Assert::IsTrue(board.isEmptyCell(1, y), L"Cell (1, 0) should be empty after eliminating cards in the column.");
			Assert::IsTrue(board.isEmptyCell(2, y), L"Cell (2, 0) should be empty after eliminating cards in the column.");
		}
		...
		TEST_METHOD(TestContainsOwnCardOnColumn)
		{
			Board board;
			Player testPlayer{ "Player1", "red" };
			size_t x = 0, y = 0;
			...
			board.placeCard(x, y, testCard);
			Card ownCard{ 1,"red", true };
			board.placeCard(x + 1, y, ownCard);
			bool containsOwnCard = board.containsOwnCardOnColumn(y, "red");
			Assert::IsTrue(containsOwnCard, L"The column 0 contains at least one red (own) card.");
		}
		...
		TEST_METHOD(TestCountOccupiedCellsOnRow)
		{
			Board board;
			size_t x = 0;
			board.placeCard(0, 0, { 1,"red",true });
			board.placeCard(0, 1, { 2,"blue",true });
			board.placeCard(0, 4, { 4,"blue",true });
			size_t count = board.countOccupiedCellsOnRow(x);
			Assert::AreEqual(count, size_t(2), L"There are two consecutive occupied cells on row 0.");
		}
		...
		TEST_METHOD(TestMoveColumn)
		{
			...
			Assert::AreEqual(static_cast<int>(4), static_cast<int>(board.GetGrid()[0][toCol]->top().GetValue()), L"The card at (0, 2) should have value 5.");
		...
		TEST_METHOD(TestMoveRow)
		{
			...
			board.placeCard(fromRow, 0, card1);
			board.placeCard(fromRow, 1, card2);

			Assert::IsFalse(board.isEmptyCell(fromRow, 0), L"Cell (0, 0) should not be empty before moving the row.");
			Assert::IsFalse(board.isEmptyCell(fromRow, 1), L"Cell (0, 1) should not be empty before moving the row.");
			Assert::IsTrue(board.isEmptyCell(toRow, 0), L"Cell (2, 0) should be empty before moving the row.");
			Assert::IsTrue(board.isEmptyCell(toRow, 1), L"Cell (2, 1) should be empty before moving the row.");

			board.moveRow(fromRow, toRow);

			Assert::IsFalse(board.isEmptyCell(toRow, 0), L"Cell (2, 0) should not be empty after moving the row.");
			Assert::IsFalse(board.isEmptyCell(toRow, 1), L"Cell (2, 1) should not be empty after moving the row.");
			Assert::AreEqual(static_cast<int>(2), static_cast<int>(board.GetGrid()[toRow][0]->top().GetValue()), L"The card at (2, 0) should have value 5.");
			Assert::AreEqual(static_cast<int>(3), static_cast<int>(board.GetGrid()[toRow][1]->top().GetValue()), L"The card at (2, 1) should have value 3.");

			Assert::IsTrue(board.isEmptyCell(fromRow, 0), L"Cell (0, 0) should be empty after moving the row.");
			Assert::IsTrue(board.isEmptyCell(fromRow, 1), L"Cell (0, 1) should be empty after moving the row.");
		}
